...
import khanhld.bk.dao.BaseDao;
import khanhld.bk.dao.BillDao;
...
public class BillDaoImpl implements BillDao {
    ...
    protected Class<Bill> type;

    public BillDaoImpl(String table, Class<Bill> type) {
        this.table = table;
        this.type = type;
    }
    ...
    public Bill getObject(ResultSet resultSet) throws SQLException {
        try {
            ...
            Bill object = (Bill) type.newInstance();//
            for (Field f : fields) {
    ...
    @Override
    public java.util.List<Bill> getList(ResultSet resultSet)  {
        java.util.List<Bill> data= new ArrayList<>();
        try {
    ...
    @Override
    public Bill insert(Bill object) throws SQLException {
    ...
    public boolean update(Bill bill) throws SQLException {
        ...
        for (int i=1; i<fileNumber;i++){
            Field f= field[i];
            f.setAccessible(true);
            prepared.setObject(i,getObject((ResultSet) bill));
        }
        Field f=field[0];
        f.setAccessible(true);
        try {
            prepared.setObject(fileNumber, f.get(bill));
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            return false;
        }
